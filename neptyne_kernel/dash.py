...
from .proxied_apis import get_api_error_service
...
_CREDENTIALS_UNDISCOVERED = object()
...
class Dash:
    ...
    def __init__(self, silent: bool = False) -> None:
        ...
        self.api_key = ""
        self.api_host = ""
        self._google_credentials = _CREDENTIALS_UNDISCOVERED
    ...
    def _discover_google_credentials(self):
        config_path = Path("~/.config/neptyne").expanduser()
        if (service_account := config_path / "service_account.json").exists():
            return self._authorize_service_account(service_account)
        elif (authorized_user := config_path / "authorized_user.json").exists():
    ...
    def _load_authorized_credentials(self, credentials: Path):
        from google.oauth2.credentials import Credentials

        self._google_credentials = Credentials.from_authorized_user_file(
            str(credentials)
        )

    def _load_oauth_credentials(self, credentials: Path):
        ...
        self._google_credentials = creds = flow.run_local_server(port=0)

        (credentials.parent / "authorized_user.json").write_text(creds.to_json("token"))
    ...
    def stop_start_streamlit(self) -> None:
        try:
            ...
            if self.cur_streamlit_info:
                ...
                # we can't stop the Server because starting it again will raise
    ...
    def gsheet_service(
        ...
        if self._gsheet_service is None:
            ...
            if self._google_credentials is _CREDENTIALS_UNDISCOVERED:
                self._discover_google_credentials()
